  ...
  async handleSubmit(evt) {
    ...
    if (!canAfford) {
      ...
      const transaction = {
        ...
        totalValue:
          Math.round(this.props.purchase.latestPrice) * 100 * Number(quantity),
      };
              ...
              <form onSubmit={this.handleSubmit}>
                <div className="field">
                  ...
                  <div className="control">
                    <input
                      className={`input ${inputColors.ticker}`}
                      type="text"
                      placeholder="Ticker"
                      name="ticker"
                ...
                <div className="field">
                  ...
                  <div className="control">
                    <input
                      className={`input ${inputColors.quantity}`}
                      type="text"
                      placeholder="Qty"
                      name="quantity"
                      onChange={this.handleChange}
                ...
                <div className="field is-grouped">
                  <div className="control">
                    <button
                      ...
                      disabled={ticker && quantity ? false : true}
                    >
                      Buy
                    </button>
                    <div>
                      <br />
                      {error && (
                        <div className="has-text-grey-darker">{error}</div>
                      )}
                    </div>
                  </div>
                </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    getPurchaseTickerDataThunk: (symbols) =>
      dispatch(getPurchaseTickerDataThunk(symbols)),
    updatePortfolioThunk: (purchasedStock) =>
      dispatch(updatePortfolioThunk(purchasedStock)),
    addTransactionThunk: (transaction) =>
